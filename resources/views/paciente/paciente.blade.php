...
@section('content')
  <div class="mb-4">
    <h1 class="section-title mb-1">Mi inicio</h1>
    <div class="muted">Hola, {{ $user->name }}.</div>
  </div>
      ...
      <div class="card h-100">
        <div class="card-body">
          <div class="stat mb-2">
            ...
            <div>
              <div class="muted small">Último control</div>
              <div class="fw-bold">
                @if($salud)
                  {{ $salud->presion_sistolica }}/{{ $salud->presion_diastolica }} mmHg
                @else
                  Sin registros
                @endif
              </div>
          ...
          @if($salud)
            <div class="row g-2">
              <div class="col-6"><span class="soft-badge d-inline-block mb-1">Glucosa</span><div class="fw-semibold">{{ $salud->glucosa_mg_dl }} mg/dL</div></div>
              <div class="col-6"><span class="soft-badge d-inline-block mb-1">FC</span><div class="fw-semibold">{{ $salud->frecuencia_cardiaca }} lpm</div></div>
              <div class="col-6"><span class="soft-badge d-inline-block mb-1">IMC</span><div class="fw-semibold">{{ $imc ?? '—' }}</div></div>
              <div class="col-6"><span class="soft-badge d-inline-block mb-1">Peso</span><div class="fw-semibold">{{ $salud->peso_kg ?? '—' }} kg</div></div>
            </div>
            <div class="muted small mt-2">{{ $salud->fecha->format('Y-m-d H:i') }}</div>
          @else
            <div class="muted">Cuando tengas controles médicos, aparecerán aquí.</div>
          @endif
        </div>
      </div>
    </div>
